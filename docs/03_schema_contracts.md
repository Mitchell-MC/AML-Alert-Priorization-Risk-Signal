...
| `silver.transaction` | one row per transaction | `transaction_id` (surrogate) | `source_account_id`, `target_account_id`, `amount`, `currency` (assumed USD), `event_time` (derived from synthetic date anchor + AMLSim `time` step), `event_date` (partition column for pruning) |
...
| Table | Grain | Key columns | Notes |
|---|---|---|---|
| `gold.entity_risk_profile` | one row per entity/account, per `as_of_date` | `entity_id`, `account_id`, `as_of_date` | `watchlist_match_summary` (struct), behavioral features: `velocity_1d/7d/30d`, `counterparty_diversity_30d`, `burst_flag`, `round_dollar_ratio`, `risky_counterparty_exposure`, `network_risk_exposure` (from `silver.network_risk_reference`, nullable — only populated for accounts that received the synthetic Elliptic linkage); `composite_risk_score`, `score_band`, `top_contributing_factors` (array of struct: factor name, value, weight/contribution). **`CREATE OR REPLACE`** every run — current-state only, see the history table below |
| `gold.entity_risk_profile_history` | one row per entity/account, per `as_of_date` **ever observed** | `entity_id`, `account_id`, `as_of_date` | Same columns as `gold.entity_risk_profile`. Append-only (`CREATE TABLE IF NOT EXISTS` + `INSERT`, never replaced) — `entity_risk_profile` itself is a `CREATE OR REPLACE` snapshot, so without this table a rebuild silently erases every prior `as_of_date`'s scores. Answers "what was this entity's score on a past date," guarded against duplicate inserts on retry by a `NOT EXISTS (entity_id, as_of_date)` check |
| `gold.prioritized_alert_queue` | one row per active alert | `alert_id` | `entity_id`, `account_id`, `priority_rank`, `composite_risk_score`, `escalation_reason` (`sanctions_hard_override` vs. `behavioral_threshold`), `status` (`new`/`reviewed`/`suppressed`, sourced from `gold.alert_disposition` — see below), `generated_at`. **`CREATE OR REPLACE`** every run |
| `gold.prioritized_alert_queue_history` | one row per `(alert_id, generated_at date)` ever observed | `alert_id`, `generated_at` | Same columns as `gold.prioritized_alert_queue`. Append-only, same rationale as `entity_risk_profile_history` — preserves which alerts were queued, at what rank and disposition, on each day the pipeline ran |
| `gold.alert_disposition` | one row per reviewer action | `alert_id`, `reviewed_at` | `status` (`new`/`reviewed`/`suppressed`), `reviewed_by`, `note`. **Insert-only** — a review is a new row, never an `UPDATE`, written via `src/aml_lakehouse/common/alert_disposition.py`. `build_gold.sql` `LEFT JOIN`s the latest disposition per `alert_id` (by `reviewed_at`) onto the freshly-scored queue so a reviewer's call survives the next `CREATE OR REPLACE` rebuild of `prioritized_alert_queue` — before this table existed, `status` was hard-coded to `'new'` in the SELECT and any reviewer action was silently lost on the next run |
...
| `gold.ops_control` | one row per pipeline batch | `pipeline_name`, `batch_id` | `business_process`, `impacted_table`, `last_successful_batch_id`, `last_good_batch_age_minutes`, `processed_row_count`, `expected_row_count`, `lag_seconds`, `freshness_status` (`fresh`/`stale`/`degraded`), `dead_letter_count`, `checkpoint_offset`, `upstream_owner`, `upstream_contact`, `runtime_seconds`, `estimated_cloud_cost_usd` — actionable business-impact alerting + ops/cost scorecard |
| `gold.month_end_reconciliation_serving` | one row per entity risk profile in serving mode | `entity_id`, `account_id`, `as_of_date` | Adds serving metadata (`as_of_batch_id`, `is_stale`, `last_good_batch_age_minutes`) so finance consumers always know freshness/lineage of month-end outputs |
